...
| 커뮤니티 | id | 조회수 | 댓글수 | 상태 |
|---|---|---|---|---|
| 에펨코리아 베스트 | `fmkorea` | ❌ 미제공 | ✅ | 수집 가능 |
| 루리웹 베스트 | `ruliweb` | ❌ 미제공 | ✅ | 수집 가능 |
| 인스티즈 | `instiz` | ✅ | ✅ | 수집 가능 |
| 디시 주식갤 | `dc_stock` | ✅ | ✅ | 수집 가능 |
| 디시 부동산갤 | `dc_realestate` | ✅ | ✅ | 수집 가능 |
| 네이버 종토방(005930) | `naver_stock` | ✅ | ❌ 미제공 | **UA 게이팅으로 차단됨** |
...
- 네이버는 브라우저가 아닌 User-Agent 에 200 응답으로 오류 페이지(약 2.7KB)를 돌려준다. robots.txt 는 이 경로를 허용하지만 서버가 UA 로 게이팅한다. **브라우저를 위장하지 않으므로** `blocked` 로 기록하고 우회하지 않는다.

로그인이나 JS 렌더가 필요해 **제외된 곳**: 네이버카페(부동산스터디·월부), 블라인드, 토스증권, 호갱노노.

## 수집 매너

- robots.txt 를 확인하고 허용 경로만 요청한다.
  - 에펨코리아: `Disallow: /` 이지만 `Allow: /best` → 베스트 목록만
  - 네이버: `Allow: /item/board.naver?code=*`, 단 `&page=` 는 금지 → 1페이지만
...
수집 출력 예:
```
수집: 에펨코리아 … ok (23건, 180ms)
수집: 루리웹 … ok (31건, 196ms)
수집: 인스티즈 … ok (15건, 451ms)
수집: 네이버 종토방 … blocked (0건, 401ms) — UA 게이팅으로 보이는 응답 (2691 bytes)
수집: 디시 주식갤 … ok (48건, 347ms)
수집: 디시 부동산갤 … ok (49건, 351ms)

수집 건수 : 166건 → 클러스터 154개
저장      : data/snapshots/2026-07-26/1900.json
```
...
collector/
  ...
  sources/
    ...
    fmkorea.js  ruliweb.js  instiz.js  naverStock.js
data/
...
> 실제 관찰: 현재 6곳은 주제 영역(유머 / 게임 / 주식 / 부동산)이 거의 겹치지 않아 **커뮤니티를 교차하는 그룹이 드물다.** 그룹핑은 주로 같은 갤러리에 연속 게시된 유사 제목 글을 묶는 데 작동한다. 교차 그룹을 늘리려면 주제가 겹치는 소스를 추가해야 한다.
